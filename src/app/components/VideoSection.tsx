...
export default function VideoSection({ video, isEven }: VideoSectionProps) {
  return (
    <section 
      className="relative min-h-[75vh] w-full flex items-center parallax-section"
      style={{
    ...
    >
      <div className="section-overlay">
        <div className="container mx-auto px-6 py-24 flex flex-col lg:flex-row items-center lg:items-start gap-12">
          <div className={`w-full lg:w-1/2 text-white ${isEven ? 'lg:order-1' : 'lg:order-2'}`}>
            ...
            <div className="bg-white/10 backdrop-blur-sm border-l-4 border-accent-color p-4 inline-block">
              <p className="text-sm opacity-90">{video.technicalInfo}</p>
            </div>
          </div>
          
          <div className={`w-full lg:w-1/2 aspect-video ${isEven ? 'lg:order-2' : 'lg:order-1'}`}>
            <div className="relative w-full h-0 pb-[56.25%] border-4 border-white">
              <iframe 
